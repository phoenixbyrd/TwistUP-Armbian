...
getchangelog() {
  ...
  messagetxt="$(wget -qO- https://twisteros.com/Patches/Twister_Armbian/messageArmbian.txt)"
...
patch2url() {
  ...
  URL="$(wget -qO- https://twisteros.com/Patches/Twister_Armbian/URLsArmbian | grep "$1" | awk '{print $2}')"
  if [ $? != 0 ] || [ -z "$URL" ];then
...
update() {
  ...
  if [ "$runmode" == 'cli-yes' ];then
    ...
    while true;do
      echo "$availablepatches" | yad --title='Twister OS Armbian Patcher' --list --separator='\n' \
        --text='The following Twister OS Armbian patches are available:' \
        --window-icon="${DIRECTORY}/icons/logo.png" \
  ...
  if [[ "$runmode" == gui* ]];then
    ...
    "$DIRECTORY/terminal-run" "trap 'echo '\''Close this terminal to exit.'\'' ; sleep infinity' EXIT
      cd "\""$DIRECTORY"\""
      $script
      yad --title='Twister OS Armbian Patcher' \
...
patchlist="$(wget -qO- https://twisteros.com/Patches/Twister_Armbian/URLsArmbian)"
...
if [[ "$runmode" == cli* ]] || [ "$runmode" == gui-update ];then
  if [ "$latestversion" == "$localversion" ];then
    #no update available
    echo -e "\nYour version of Twister OS Armbian is fully up to date already.\nExiting now."
    exit 0
  ...
  output="$(yad --form --text='Twister OS Armbian can be updated.' \
